...
/*export*/ template <class T>
void Matrix<T>::resize(const size_t rows, const size_t columns, const T default_value) {//default_value缺省时是多少
  assert ( rows > 0 && columns > 0 && "Columns and rows must exist." );//assert用于诊断，如果参数为真无任何操作，为假则输出错误信息

  if ( m_matrix == nullptr ) {//矩阵matrix
    // alloc arrays
