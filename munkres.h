...
public:
     ...
     *
     */
    void solve(Matrix<Data> &m) {//Matrix是一个类模板，参数m已是取反的iou
        const size_t rows = m.rows(),
                columns = m.columns(),
                size = std::max(rows, columns);

#ifdef DEBUG
        std::cout << "Munkres input: " << m << std::endl;//用于调试输出iou矩阵
#endif
        ...
        if ( rows != columns ) {
            // If the input matrix isn't square, make it square and fill the empty values with the largest value present in the matrix.
        ...
        // If there were any infinities, replace them with a value greater than the maximum value in the matrix.
        replace_infinites(matrix);//这一步是为什么
    ...
    static void replace_infinites(Matrix<Data> &matrix) {
      ...
      constexpr auto infinity = std::numeric_limits<double>::infinity();//浮点数表示的正无穷大
    ...
    static void minimize_along_direction(Matrix<Data> &matrix, const bool over_columns) {
      ...
      for ( size_t i = 0 ; i < outer_size ; i++ ) {
        ...
        // As long as the current minimum is greater than zero, keep looking for the minimum. Start at one because we already have the 0th value in min.
